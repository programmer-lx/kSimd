...
#define KSIMD_DISPATCH_THIS_FILE "lerp.cpp" // this file
...
namespace KSIMD_DYN_INSTRUCTION
{
    ...
    ) noexcept
    {
        ...
        {
            op::store(out + i, math::lerp(op::load(a + i), op::load(b + i), op::load(t + i)));
        }
        for (; i < size; ++i)
        {
            out[i] = math::lerp(a[i], b[i], t[i]);
        }
    }
}
...
{
    KSIMD_DYN_CALL(kernel_impl)(v, b, t, out, size);
}
#endif