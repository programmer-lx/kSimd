...
#define KSIMD_DISPATCH_THIS_FILE "safe_clamp.cpp" // this file
...
namespace KSIMD_DYN_INSTRUCTION
{
    ...
    ) noexcept
    {
        ...
        {
            op::store(out + i, math::safe_clamp(op::load(v + i), op::load(min + i), op::load(max + i)));
        }
        for (; i < size; ++i)
        {
            out[i] = math::safe_clamp(v[i], min[i], max[i]);
        }
    }
}
