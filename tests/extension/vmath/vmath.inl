...
namespace KSIMD_DYN_INSTRUCTION {
    ...
    void lerp() noexcept {
        ...
        do { \
            ns::store(t,act_buf, actual); \
            ns::store(t,exp_buf, expected); \
            perform_lerp_check(act_buf, exp_buf, Lanes, msg); \
        } while (0)

        const batch_t a = ns::set(t, FLOAT_T(10.0));
        const batch_t b = ns::set(t, FLOAT_T(20.0));

        // --- 1. 标准范围内插值 ---
        CHECK(ns::vmath::lerp(t, a, b, ns::set(t, FLOAT_T(0.0))), a, "t=0");
        CHECK(ns::vmath::lerp(t, a, b, ns::set(t, FLOAT_T(1.0))), b, "t=1");
        CHECK(ns::vmath::lerp(t, a, b, ns::set(t, FLOAT_T(0.5))), ns::set(t, FLOAT_T(15.0)), "t=0.5");

        // --- 2. 外插测试 (Extrapolation) ---
        // t = -0.3  => 10 + (-0.3 * 10) = 7
        CHECK(ns::vmath::lerp(t, a, b, ns::set(t, FLOAT_T(-0.3))), ns::set(t, FLOAT_T(7.0)), "t < 0");
        // t = 1.2   => 10 + (1.2 * 10) = 22
        CHECK(ns::vmath::lerp(t, a, b, ns::set(t, FLOAT_T(1.2))),  ns::set(t, FLOAT_T(22.0)), "t > 1");

        // --- 3. 负数范围与反向插值 ---
        // a=10, b=-10, t=0.2 => 10 + 0.2*(-20) = 6
        CHECK(ns::vmath::lerp(t, a, ns::set(t, FLOAT_T(-10.0)), ns::set(t, FLOAT_T(0.2))), ns::set(t, FLOAT_T(6.0)), "mixed sign");

        // --- 4. 极端值测试 ---
        // 同值插值
        CHECK(ns::vmath::lerp(t, a, a, ns::set(t, FLOAT_T(0.7))), a, "a == b");

        // 零区间插值 (a=0, b=0)
        batch_t zero = ns::set(t, FLOAT_T(0.0));
        CHECK(ns::vmath::lerp(t, zero, zero, ns::set(t, FLOAT_T(0.5))), zero, "zero range");

        // NaN 传播测试 (基于你对 clamp 的 NaN 处理逻辑，lerp 也应遵循)
        if constexpr (std::numeric_limits<FLOAT_T>::has_quiet_NaN) {
            batch_t nan_v = ns::set(t, std::numeric_limits<FLOAT_T>::quiet_NaN());
            // 如果 t 是 NaN，结果应当是 NaN
            CHECK(ns::vmath::lerp(t, a, b, nan_v), nan_v, "NaN propagation in t");
        }
...
namespace KSIMD_DYN_INSTRUCTION {
    ...
    void clamp_test() noexcept {
        ...
        const batch_t v_mid = ns::set(t, FLOAT_T(15.0));
        const batch_t v_low = ns::set(t, FLOAT_T(5.0));
        const batch_t v_high = ns::set(t, FLOAT_T(25.0));
        const batch_t min_v = ns::set(t, FLOAT_T(10.0));
        const batch_t max_v = ns::set(t, FLOAT_T(20.0));
        const batch_t nan_v = ns::set(t, ksimd::QNaN<FLOAT_T>);
        ...
        // 测试原值在范围内
        CLAMP_CHECK(ns::vmath::clamp<Opt::Native>(t,v_mid, min_v, max_v), v_mid, "Native: within range");
        // 测试下限触发
        CLAMP_CHECK(ns::vmath::clamp<Opt::Native>(t,v_low, min_v, max_v), min_v, "Native: lower bound");
        // 测试上限触发
        CLAMP_CHECK(ns::vmath::clamp<Opt::Native>(t,v_high, min_v, max_v), max_v, "Native: upper bound");
        // 测试 v 是 NaN 的传播 (利用你提到的 v 在右侧特性)
        CLAMP_CHECK(ns::vmath::clamp<Opt::Native>(t,nan_v, min_v, max_v), nan_v, "Native: v is NaN propagation");
        ...
        CLAMP_CHECK(ns::vmath::clamp<Opt::Native>(t,nan_v, nan_v, max_v), nan_v, "Native: min and v are NaN");
