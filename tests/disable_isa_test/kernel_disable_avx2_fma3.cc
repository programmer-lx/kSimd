...
#pragma message("disable avx2_fma3")

namespace KSIMD_DYN_INSTRUCTION
{
    void kernel_disable_avx2_fma3(size_t index)
    {
...
#if KSIMD_ONCE
KSIMD_DYN_DISPATCH_FUNC(kernel_disable_avx2_fma3);
void kernel_disable_avx2_fma3()
{
    volatile size_t table_size = std::size(KSIMD_DETAIL_PFN_TABLE_FULL_NAME(kernel_disable_avx2_fma3));
    if (table_size != 2)
    {
        throw std::runtime_error("we should disable AVX2_FMA3");
    }
    ...
    // try call
    for (size_t i = 0; i < std::size(KSIMD_DETAIL_PFN_TABLE_FULL_NAME(kernel_disable_avx2_fma3)); ++i)
    {
        KSIMD_DETAIL_PFN_TABLE_FULL_NAME(kernel_disable_avx2_fma3)[i](i);
    }
}
#endif
