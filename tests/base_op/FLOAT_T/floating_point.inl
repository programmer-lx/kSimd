...
namespace KSIMD_DYN_INSTRUCTION
{
    ...
    void round_down() noexcept
    {
        ...
        // 常规：正数向下取整
        op::store(test, op::round<op::RoundingMode::Down>(op::set(FLOAT_T(2.7))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(2.0));

        // 常规：负数向下取整 (floor(-2.1) = -3)
        op::store(test, op::round<op::RoundingMode::Down>(op::set(FLOAT_T(-2.1))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(-3.0));

        // 边界：Inf 和 NaN
        op::store(test, op::round<op::RoundingMode::Down>(op::set(inf<FLOAT_T>)));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_TRUE(std::isinf(test[i]) && test[i] > 0);

        op::store(test, op::round<op::RoundingMode::Down>(op::set(qNaN<FLOAT_T>)));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_TRUE(std::isnan(test[i]));
    }
}
#if KSIMD_ONCE
...
namespace KSIMD_DYN_INSTRUCTION
{
    ...
    void round_up() noexcept
    {
        ...
        // 常规：正数向上取整
        op::store(test, op::round<op::RoundingMode::Up>(op::set(FLOAT_T(2.1))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(3.0));

        // 常规：负数向上取整 (ceil(-2.7) = -2)
        op::store(test, op::round<op::RoundingMode::Up>(op::set(FLOAT_T(-2.7))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(-2.0));

        // 边界：大数值 (超过有效尾数范围应保持原样)
        FLOAT_T big_val = FLOAT_T(1LL << (std::is_same_v<FLOAT_T, float> ? 25 : 54));
        op::store(test, op::round<op::RoundingMode::Up>(op::set(big_val + FLOAT_T(0.5))));
...
namespace KSIMD_DYN_INSTRUCTION
{
    ...
    void round_to_zero() noexcept
    {
        ...
        #define check(input, expected, msg) \
            do { \
                op::store(test, op::round<op::RoundingMode::ToZero>(op::set(input))); \
                for (size_t i = 0; i < Lanes; ++i) { \
...
namespace KSIMD_DYN_INSTRUCTION
{
    ...
    void round_nearest() noexcept
    {
        ...
        // 2.5 -> 2.0 (最近偶数)
        op::store(test, op::round<op::RoundingMode::Nearest>(op::set(FLOAT_T(2.5))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(2.0));

        // 3.5 -> 4.0 (最近偶数)
        op::store(test, op::round<op::RoundingMode::Nearest>(op::set(FLOAT_T(3.5))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(4.0));

        // 常规数值
        op::store(test, op::round<op::RoundingMode::Nearest>(op::set(FLOAT_T(-2.1))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(-2.0));

        op::store(test, op::round<op::RoundingMode::Nearest>(op::set(FLOAT_T(-2.9))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(-3.0));

        // 边界：NaN
        op::store(test, op::round<op::RoundingMode::Nearest>(op::set(qNaN<FLOAT_T>)));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_TRUE(std::isnan(test[i]));
    }
}
#if KSIMD_ONCE
...
namespace KSIMD_DYN_INSTRUCTION
{
    ...
    void round() noexcept
    {
        ...
        // 正数 0.5 进位
        op::store(test, op::round<op::RoundingMode::Round>(op::set(FLOAT_T(2.5))));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_EQ(test[i], FLOAT_T(3.0));
        ...
        // 测试略小于 0.5 的情况 (应舍向 0)
        op::store(test, op::round<op::RoundingMode::Round>(op::set(FLOAT_T(0.5) - eps)));
        ...
        op::store(test, op::round<op::RoundingMode::Round>(op::set(FLOAT_T(-0.0))));
        for (size_t i = 0; i < Lanes; ++i) {
        ...
        // 负的小数 (未达进位点) 应该保持负零符号
        op::store(test, op::round<op::RoundingMode::Round>(op::set(FLOAT_T(-0.1))));
        for (size_t i = 0; i < Lanes; ++i) {
            EXPECT_EQ(test[i], FLOAT_T(0.0));
            EXPECT_TRUE(std::signbit(test[i]));
        }
        ...
        op::store(test, op::round<op::RoundingMode::Round>(op::set(big_val)));
        for (size_t i = 0; i < Lanes; ++i) FLOAT_T_EQ(test[i], big_val);

        // 大值 + 1.0 同样
        op::store(test, op::round<op::RoundingMode::Round>(op::set(big_val + FLOAT_T(1.0))));
        for (size_t i = 0; i < Lanes; ++i) FLOAT_T_EQ(test[i], big_val + FLOAT_T(1.0));


        // --- 5. 特殊值处理 ---
        // 正负无穷
        op::store(test, op::round<op::RoundingMode::Round>(op::set(inf<FLOAT_T>)));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_TRUE(std::isinf(test[i]));
        ...
        // NaN 处理
        op::store(test, op::round<op::RoundingMode::Round>(op::set(qNaN<FLOAT_T>)));
        for (size_t i = 0; i < Lanes; ++i) EXPECT_TRUE(std::isnan(test[i]));
    }
}
#if KSIMD_ONCE
