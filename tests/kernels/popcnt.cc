...
ks_uint64_t test_popcnt_ref(T x)
{
    ks_uint64_t cnt = 0;
...
TEST(popcnt, popcnt8_soft)
{
    ...
    for (int i = 0; i <= 255; ++i)
    {
        ...
        ks_uint64_t expected = test_popcnt_ref(val);
...
static ks_uint64_t reference_popcnt(const void* buffer, size_t size) {
...
TEST(popcnt, popcnt_buffer_soft)
{
    ...
    ks_uint64_t expected_rand = reference_popcnt(random_buffer.data(), large_size);
...
{
    {
        ...
        ks_uint64_t expected = reference_popcnt(large_buffer.data(), large_len);
...

    {
        ...
        // 我们故意在不同的偏移量上调用函数
        ks_uint64_t ref_val = reference_popcnt(pattern, pattern_len);
