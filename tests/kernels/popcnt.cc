...
static size_t reference_popcnt(const void* buffer, size_t size) {
...
TEST(popcnt, popcnt_buffer_soft)
{
    ...
    size_t expected_rand = reference_popcnt(random_buffer.data(), large_size);
...
{
    {
        ...
        size_t expected = reference_popcnt(large_buffer.data(), large_len);
...

    {
        ...
        // 我们故意在不同的偏移量上调用函数
        size_t ref_val = reference_popcnt(pattern, pattern_len);
