...
#pragma region--- types ---
    namespace detail
    {
        ...
        template<typename Tag>
            requires(is_tag_scalable_full<Tag> && (is_tag_float_16bits<Tag> || is_tag_float_32bits<Tag>))
        KSIMD_API(uint64_t) lane_mask(Tag t) noexcept
        {
            const size_t l = lanes(t);
            if (l >= 64) [[unlikely]]
                return UINT64_MAX;
            return (UINT64_C(1) << l) - UINT64_C(1);
        }

        template<typename Tag>
            requires(is_tag_scalable_full<Tag> && (is_tag_float_16bits<Tag> || is_tag_float_32bits<Tag>))
        KSIMD_API(uint64_t) mask_to_bits(Tag t, svbool_t m) noexcept
        {
            std::array<uint32_t, SVE_MAX_LANES_F32> tmp{};
            svuint32_t ones = svdup_n_u32(UINT32_MAX);
            svuint32_t zeros = svdup_n_u32(0);
            svuint32_t packed = svsel_u32(m, ones, zeros);
            svst1_u32(svptrue_b32(), tmp.data(), packed);

            uint64_t bits = 0;
            const size_t l = lanes(t);
            for (size_t i = 0; i < l; ++i)
            {
                bits |= static_cast<uint64_t>(tmp[i] != 0) << i;
            }
            return bits & lane_mask(t);
        }

        template<typename Tag>
            requires(is_tag_scalable_full<Tag> && (is_tag_float_16bits<Tag> || is_tag_float_32bits<Tag>))
        KSIMD_API(svbool_t) bits_to_mask(Tag t, uint64_t bits) noexcept
        {
            std::array<uint32_t, SVE_MAX_LANES_F32> tmp{};
            const size_t l = lanes(t);
            for (size_t i = 0; i < l; ++i)
            {
                tmp[i] = ((bits >> i) & UINT64_C(1)) ? UINT32_MAX : 0u;
            }
            svuint32_t v = svld1_u32(svptrue_b32(), tmp.data());
            return svcmpne_n_u32(svptrue_b32(), v, 0u);
        }

        template<typename Tag>
            requires(is_tag_scalable_full<Tag> && is_tag_float_32bits<Tag>)
        KSIMD_API(svfloat32_t) load_f32_partial(const float* mem, size_t count) noexcept
        {
        ...
        template<typename Tag>
            requires(is_tag_scalable_full<Tag> && is_tag_float_32bits<Tag>)
        KSIMD_API(void) store_f32_partial(float* mem, svfloat32_t v, size_t count) noexcept
        {
            svbool_t pg = svwhilelt_b32(static_cast<uint64_t>(0), static_cast<uint64_t>(count));
            svst1_f32(pg, mem, v);
        }
...
#pragma region--- any types/float32 ---
    ...
    KSIMD_API(Batch<Tag>) loadu_partial(Tag t, const tag_scalar_t<Tag>* mem, size_t count) noexcept
    {
        ...
        return detail::load_f32_partial<Tag>(reinterpret_cast<const float*>(mem), count);
    }
    ...
    {
        count = count > lanes(t) ? lanes(t) : count;
        detail::store_f32_partial<Tag>(reinterpret_cast<float*>(mem), v, count);
    }
    ...
    {
        return bit_or(Tag{}, bit_and(Tag{}, _if, _then), bit_and_not(Tag{}, _if, _else));
    }
    ...
    template<typename Tag>
        requires(is_tag_float_32bits<Tag> && is_tag_scalable_full<Tag>)
    KSIMD_API(MaskBitset<Tag>) reduce_mask(Tag t, Mask<Tag> mask) noexcept
    {
        return detail::mask_to_bits(t, mask);
    }
