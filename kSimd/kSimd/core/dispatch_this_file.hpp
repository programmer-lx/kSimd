#pragma once

#include "impl/dispatch.hpp"

#if !defined(KSIMD_DISPATCH_THIS_FILE)
    #error we should defined KSIMD_DISPATCH_THIS_FILE before include <kSimd/core/dispatch_this_file.hpp>
#endif

#define KSIMD_DETAIL_CHECK_DISPATCH_FILE_INCLUDED

#undef KSIMD_ONCE
#define KSIMD_ONCE 0

// AVX512 V4
#if KSIMD_INSTRUCTION_FEATURE_X86_V4
    // KSIMD_DYN_INSTRUCTION
    #undef KSIMD_DYN_INSTRUCTION
    #define KSIMD_DYN_INSTRUCTION KSIMD_DYN_INSTRUCTION_X86_V4

    // KSIMD_DYN_FUNC_ATTR
    #undef KSIMD_DYN_FUNC_ATTR
    #define KSIMD_DYN_FUNC_ATTR KSIMD_DYN_FUNC_ATTR_X86_V4

    // KSIMD_DYN_DISPATCH_LEVEL
    #undef KSIMD_DYN_DISPATCH_LEVEL
    #define KSIMD_DYN_DISPATCH_LEVEL KSIMD_DYN_DISPATCH_LEVEL_X86_V4

    // dispatch if not fallback
    #if (KSIMD_INSTRUCTION_FEATURE_X86_V4 != KSIMD_INSTRUCTION_FEATURE_FALLBACK_VALUE)
        #include KSIMD_DISPATCH_THIS_FILE
    #endif
#endif

// AVX2 + FMA3 + F16C V3
#if KSIMD_INSTRUCTION_FEATURE_X86_V3
    // KSIMD_DYN_INSTRUCTION
    #undef KSIMD_DYN_INSTRUCTION
    #define KSIMD_DYN_INSTRUCTION KSIMD_DYN_INSTRUCTION_X86_V3

    // KSIMD_DYN_FUNC_ATTR
    #undef KSIMD_DYN_FUNC_ATTR
    #define KSIMD_DYN_FUNC_ATTR KSIMD_DYN_FUNC_ATTR_X86_V3

    // KSIMD_DYN_DISPATCH_LEVEL
    #undef KSIMD_DYN_DISPATCH_LEVEL
    #define KSIMD_DYN_DISPATCH_LEVEL KSIMD_DYN_DISPATCH_LEVEL_X86_V3

    // dispatch if not fallback
    #if (KSIMD_INSTRUCTION_FEATURE_X86_V3 != KSIMD_INSTRUCTION_FEATURE_FALLBACK_VALUE)
        #include KSIMD_DISPATCH_THIS_FILE
    #endif
#endif

// SSE4.1 V2
#if KSIMD_INSTRUCTION_FEATURE_X86_V2
    // KSIMD_DYN_INSTRUCTION
    #undef KSIMD_DYN_INSTRUCTION
    #define KSIMD_DYN_INSTRUCTION KSIMD_DYN_INSTRUCTION_X86_V2

    // KSIMD_DYN_FUNC_ATTR
    #undef KSIMD_DYN_FUNC_ATTR
    #define KSIMD_DYN_FUNC_ATTR KSIMD_DYN_FUNC_ATTR_X86_V2

    // KSIMD_DYN_DISPATCH_LEVEL
    #undef KSIMD_DYN_DISPATCH_LEVEL
    #define KSIMD_DYN_DISPATCH_LEVEL KSIMD_DYN_DISPATCH_LEVEL_X86_V2

    // dispatch if not fallback
    #if (KSIMD_INSTRUCTION_FEATURE_X86_V2 != KSIMD_INSTRUCTION_FEATURE_FALLBACK_VALUE)
        #include KSIMD_DISPATCH_THIS_FILE
    #endif
#endif


// arm SVE
#if KSIMD_INSTRUCTION_FEATURE_SVE
    // KSIMD_DYN_INSTRUCTION
    #undef KSIMD_DYN_INSTRUCTION
    #define KSIMD_DYN_INSTRUCTION KSIMD_DYN_INSTRUCTION_SVE

    // KSIMD_DYN_FUNC_ATTR
    #undef KSIMD_DYN_FUNC_ATTR
    #define KSIMD_DYN_FUNC_ATTR KSIMD_DYN_FUNC_ATTR_SVE

    // KSIMD_DYN_DISPATCH_LEVEL
    #undef KSIMD_DYN_DISPATCH_LEVEL
    #define KSIMD_DYN_DISPATCH_LEVEL KSIMD_DYN_DISPATCH_LEVEL_SVE

    // dispatch if not fallback
    #if (KSIMD_INSTRUCTION_FEATURE_SVE != KSIMD_INSTRUCTION_FEATURE_FALLBACK_VALUE)
        #include KSIMD_DISPATCH_THIS_FILE
    #endif
#endif


// arm NEON
#if KSIMD_INSTRUCTION_FEATURE_NEON
    // KSIMD_DYN_INSTRUCTION
    #undef KSIMD_DYN_INSTRUCTION
    #define KSIMD_DYN_INSTRUCTION KSIMD_DYN_INSTRUCTION_NEON

    // KSIMD_DYN_FUNC_ATTR
    #undef KSIMD_DYN_FUNC_ATTR
    #define KSIMD_DYN_FUNC_ATTR KSIMD_DYN_FUNC_ATTR_NEON

    // KSIMD_DYN_DISPATCH_LEVEL
    #undef KSIMD_DYN_DISPATCH_LEVEL
    #define KSIMD_DYN_DISPATCH_LEVEL KSIMD_DYN_DISPATCH_LEVEL_NEON

    // dispatch if not fallback
    #if (KSIMD_INSTRUCTION_FEATURE_NEON != KSIMD_INSTRUCTION_FEATURE_FALLBACK_VALUE)
        #include KSIMD_DISPATCH_THIS_FILE
    #endif
#endif


// Scalar
#if KSIMD_INSTRUCTION_FEATURE_SCALAR
    // KSIMD_DYN_INSTRUCTION
    #undef KSIMD_DYN_INSTRUCTION
    #define KSIMD_DYN_INSTRUCTION KSIMD_DYN_INSTRUCTION_SCALAR

    // KSIMD_DYN_FUNC_ATTR
    #undef KSIMD_DYN_FUNC_ATTR
    #define KSIMD_DYN_FUNC_ATTR KSIMD_DYN_FUNC_ATTR_SCALAR

    // KSIMD_DYN_DISPATCH_LEVEL
    #undef KSIMD_DYN_DISPATCH_LEVEL
    #define KSIMD_DYN_DISPATCH_LEVEL KSIMD_DYN_DISPATCH_LEVEL_SCALAR

    // dispatch if not fallback
    #if (KSIMD_INSTRUCTION_FEATURE_SCALAR != KSIMD_INSTRUCTION_FEATURE_FALLBACK_VALUE)
        #include KSIMD_DISPATCH_THIS_FILE
    #endif
#endif


// last dispatch
// once
#undef KSIMD_ONCE
#define KSIMD_ONCE 1

#undef KSIMD_DISPATCH_THIS_FILE
