...
#define KSIMD_IOTA 3.0f, 2.0f, 1.0f, 0.0f
...
namespace x86_vector128
{
    ...
    {
        return { _mm_add_ps(lhs.v[0], rhs.v[0]) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator-(Batch<float32, reg_count> lhs, Batch<float32, reg_count> rhs) noexcept
    {
        return { _mm_sub_ps(lhs.v[0], rhs.v[0]) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator*(Batch<float32, reg_count> lhs, Batch<float32, reg_count> rhs) noexcept
    {
        return { _mm_mul_ps(lhs.v[0], rhs.v[0]) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator/(Batch<float32, reg_count> lhs, Batch<float32, reg_count> rhs) noexcept
    {
        return { _mm_div_ps(lhs.v[0], rhs.v[0]) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator-(Batch<float32, reg_count> v) noexcept
    {
        return { _mm_xor_ps(v.v[0], _mm_set1_ps(SignBitMask<float32>)) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator&(Batch<float32, reg_count> lhs, Batch<float32, reg_count> rhs) noexcept
    {
        return { _mm_and_ps(lhs.v[0], rhs.v[0]) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator|(Batch<float32, reg_count> lhs, Batch<float32, reg_count> rhs) noexcept
    {
        return { _mm_or_ps(lhs.v[0], rhs.v[0]) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator^(Batch<float32, reg_count> lhs, Batch<float32, reg_count> rhs) noexcept
    {
        return { _mm_xor_ps(lhs.v[0], rhs.v[0]) };
    }

    template<size_t reg_count>
    KSIMD_API(Batch<float32, reg_count>) operator~(Batch<float32, reg_count> v) noexcept
    {
        return { _mm_xor_ps(v.v[0], _mm_set1_ps(OneBlock<float32>)) };
    }
...
struct BaseOp<SimdInstruction::KSIMD_DYN_INSTRUCTION_SSE, float32>
{
    ...
    KSIMD_API(batch_t) mask_load(const float32* mem, mask_t mask) noexcept
    {
        ...
        const int32 m = _mm_movemask_ps(mask.m[0]); // [3:0] 有效
    ...
    KSIMD_API(batch_t) mask_load(const float32* mem, mask_t mask, batch_t default_value) noexcept
    {
        ...
        return { _mm_or_ps(loaded.v[0], _mm_andnot_ps(mask.m[0], default_value.v[0])) };
    }
    ...
    KSIMD_API(void) mask_store(float32* mem, batch_t v, mask_t mask) noexcept
    {
        ...
        if (m & 0b0001)
        {
            _mm_store_ss(mem, v.v[0]);
        }
        if (m & 0b0010)
        {
            __m128 tmp = _mm_shuffle_ps(v.v[0], v.v[0], _MM_SHUFFLE(1, 1, 1, 1));
            _mm_store_ss(mem + 1, tmp);
        }
        if (m & 0b0100)
        {
            __m128 tmp = _mm_shuffle_ps(v.v[0], v.v[0], _MM_SHUFFLE(2, 2, 2, 2));
            _mm_store_ss(mem + 2, tmp);
        }
        if (m & 0b1000)
        {
            __m128 tmp = _mm_shuffle_ps(v.v[0], v.v[0], _MM_SHUFFLE(3, 3, 3, 3));
    ...
    KSIMD_API(batch_t) sequence(float32 base) noexcept
    {
        ...
        return { _mm_add_ps(iota, _mm_set1_ps(base)) };
    }
    ...
    {
        return { _mm_add_ps(_mm_mul_ps(a.v[0], b.v[0]), c.v[0]) };
    }

    KSIMD_API(batch_t) sqrt(batch_t v) noexcept
    {
        return { _mm_sqrt_ps(v.v[0]) };
    }

    KSIMD_API(batch_t) rsqrt(batch_t v) noexcept
    {
        return { _mm_rsqrt_ps(v.v[0]) };
    }

    KSIMD_API(batch_t) abs(batch_t v) noexcept
    {
        return { _mm_and_ps(v.v[0], _mm_set1_ps(SignBitClearMask<float32>)) };
    }
    ...
    KSIMD_API(mask_t) any_finite(batch_t lhs, batch_t rhs) noexcept
    {
        ...
        __m128 combined = _mm_and_ps(lhs.v[0], rhs.v[0]);
    ...
    KSIMD_API(mask_t) all_finite(batch_t lhs, batch_t rhs) noexcept
    {
        ...
        __m128 l_finite = _mm_cmplt_ps(_mm_and_ps(lhs.v[0], abs_mask), inf);
        __m128 r_finite = _mm_cmplt_ps(_mm_and_ps(rhs.v[0], abs_mask), inf);
    ...
    {
        return { _mm_andnot_ps(lhs.v[0], rhs.v[0]) };
    }
    ...
    {
        return { _mm_or_ps(_mm_and_ps(mask.v[0], a.v[0]), _mm_andnot_ps(mask.v[0], b.v[0])) };
    }

    KSIMD_API(batch_t) mask_select(mask_t mask, batch_t a, batch_t b) noexcept
    {
        return { _mm_or_ps(_mm_and_ps(mask.m[0], a.v[0]), _mm_andnot_ps(mask.m[0], b.v[0])) };
    }
};
#undef KSIMD_API
    ...
    : BaseOp<SimdInstruction::KSIMD_DYN_INSTRUCTION_SSSE3, float32>
{
    KSIMD_API(batch_t) mask_select(mask_t mask, batch_t a, batch_t b) noexcept
    {
        return { _mm_blendv_ps(b.v[0], a.v[0], mask.m[0]) };
    }
    ...
    KSIMD_API(batch_t) round(batch_t v) noexcept
    {
        ...
        {
            return { _mm_round_ps(v.v[0], _MM_FROUND_TO_POS_INF | _MM_FROUND_NO_EXC) };
        }
        else if constexpr (mode == RoundingMode::Down)
        {
            return { _mm_round_ps(v.v[0], _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC) };
        }
        else if constexpr (mode == RoundingMode::Nearest)
        {
            return { _mm_round_ps(v.v[0], _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC) };
        }
        else if constexpr (mode == RoundingMode::Round)
        {
            // 提取符号位，如果v是负数，则sign_mask为0b1000...，如果v是正数，则sign_mask为0b0000...
            __m128 sign_mask = _mm_and_ps(v.v[0], _mm_set1_ps(SignBitMask<float32>));

            // 构造一个具有相同符号的0.5 (0x1.0p-1f == 0.5f 16进制精确表示)
            __m128 half = _mm_or_ps(_mm_set1_ps(0x1.0p-1f), sign_mask);

            return { _mm_round_ps(_mm_add_ps(v.v[0], half), _MM_FROUND_TO_ZERO | _MM_FROUND_NO_EXC) };
        }
        else /* if constexpr (mode == RoundingMode::ToZero) */
        {
            return { _mm_round_ps(v.v[0], _MM_FROUND_TO_ZERO | _MM_FROUND_NO_EXC) };
        }
    }
};
#undef KSIMD_API
