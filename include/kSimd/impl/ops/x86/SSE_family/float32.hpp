...
struct SimdOp<SimdInstruction::SSE, float32>
{
    ...
    KSIMD_OP_SIG_SSE(mask_t, test_load_mask, (const float32* mem))
    {
    ...
    KSIMD_OP_SIG_SSE(batch_t, mask_loadu, (const float32* mem, mask_t mask))
    {
        uint32 m = _mm_movemask_ps(mask.m); // 仅 [3:0] 有效
        alignas(BatchAlignment) float32 tmp[Lanes]{};
    ...
    KSIMD_OP_SIG_SSE(void, mask_store, (float32* mem, batch_t v, mask_t mask))
    {
        alignas(BatchAlignment) float32 tmp[Lanes]{};
    ...
    KSIMD_OP_SIG_SSE(void, mask_storeu, (float32* mem, batch_t v, mask_t mask))
    {
        alignas(BatchAlignment) float32 tmp[Lanes]{};
        _mm_store_ps(tmp, v.v);
