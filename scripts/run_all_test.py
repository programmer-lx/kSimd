...
def main():
    try:
        ...
        """
        编译选项: 格式 (build config, -DKSIMD_TEST_OPTION[od, o2, gl])
        """
        ...
        for build_cfg, test_option in build_options:
            for name, c_compiler, cxx_compiler, build_subdir in configs:
                current_build_dir = build_base / (OS_NAME + '_' + build_subdir + '_' + build_cfg + '_' + test_option)
                ...
                run_command([
                    ...
                    f"-DKSIMD_TEST_OPTION={test_option}"
                ])
